...
import { faPlus, faHouse, faImage } from '@fortawesome/free-solid-svg-icons';
...
import { datasetToolProjectAPI_B } from '../APIPath';
...
const UpsertProjectDialog = (props: UpsertProjectDialogProps) => {
    ...
    const handleSubmit = () => {
        ...
        const postData = {
            ...
            is_high_aspect: (currentProject?.project_high_aspect) ? true : false,
            annotation: myNote,
        ...
        <>
            ...
            <ThemeProvider theme={theme}>
                <Dialog open={!!openUpsertDialog} className={cx(classes.customDialog)} onClose={handleClose}>
                    <div className={currentProject?.project_high_aspect ? "dialog-container-aspect" : "dialog-container"}>


                        {
                            (currentProject?.project_high_aspect) ?
                                <div className='d-flex flex-row justify-content-start align-items-center gap-1' style={{ height: 90 }}>
                                    <div className='my-project-type-icon-2-aspect'>
                                        <FontAwesomeIcon icon={faScissors} style={{ width: 25, height: 25 }} />
                                    </div>
                                    <h4 style={{ margin: 5 }}>{currentProject?.project_uuid ? `Edit ${(currentProject?.project_high_aspect) ? 'Finger' : 'General'} Project` : `Add ${(currentProject?.project_high_aspect) ? 'Finger' : 'General'} Project`}</h4>

                                </div>
                                :
                                <div className='d-flex flex-row justify-content-start align-items-center gap-1' style={{ height: 90 }}>
                                    <div className='my-project-type-icon-2'>
                                        <FontAwesomeIcon icon={faImage} style={{ width: 25, height: 25 }} />
                                    </div>
                                    <h4 style={{ margin: 5 }}>{currentProject?.project_uuid ? `Edit ${(currentProject?.project_high_aspect) ? 'Finger' : 'General'} Project` : `Add ${(currentProject?.project_high_aspect) ? 'Finger' : 'General'} Project`}</h4>
                        ...
                        <div>
                            ...
                            <div className="lower-right-button-container mt-2">
                                ...
                                <CustomButton name={(currentProject?.project_high_aspect) ? "view-aspect" : "view"} text="Save" width={100} onClick={() => handleSubmit()}></CustomButton>
                            </div>
                        </div>
                    </div>
                </Dialog>
