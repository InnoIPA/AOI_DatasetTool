...
const ResultTableListAspect = (props) => {

    const tableColumnWidth = [80, 150, 180, 150, 150, 150, 150, 100, 50];
    ...
    const handleLabelToggle = (segmentUuid) => {
        props.onChange(segmentUuid);
    }
        ...
        <ThemeProvider theme={theme}>
                ...
                <ModalDialog style={{ width: '100vw', height: '100vh', backgroundColor: '#ffffffcc' }} layout='top'>
                    <div className='d-flex flex-row align-items-center align-items-center gap-4' style={{ width: '100vw', height: '100vh' }}>
                        <div style={{ backgroundColor: 'transparent', width: '50vw', height: '100vh' }} className='d-flex justify-content-center align-items-center'>
                            <img src={datasetSegImgAPI(hoverItem1)} style={{ maxWidth: '46vw', maxHeight: '92vh' }} />
                        </div>
                        <div style={{ backgroundColor: 'transparent', width: '50vw', height: '100vh' }} className='d-flex justify-content-center align-items-center'>
                            <img src={datasetSegImgAPI(hoverItem2)} style={{ maxWidth: '46vw', maxHeight: '92vh' }} />
                        </div>
                    </div>
            ...
            <div className='my-table'>
                <div className={(true) ? 'my-thead' : 'my-thead-shadow'} style={{ backgroundColor: "#FAFAFD" }}>
                    ...
                    <div className='my-thead-th' style={{ width: tableColumnWidth[7] }}>Seg No.</div>

                </div>
                <div className='my-tbody' style={{ height: 650 }}>
                    {
                        props.data.map((item, index) => (
                            <div className={`my-tbody-row-${(index % 2 === 1) ? "1" : "2"}`} key={`${item.segmentUuid}_${index}`}>
                                ...
                                <div className='my-tbody-td' style={{ width: tableColumnWidth[3] }} >
                                    <div className={(item.label === "PASS") ? 'my-thumb-image-container-pass' : 'my-thumb-image-container-ng'} onMouseEnter={() => { setHoverItem1(item.segmentUuid); setHoverItem2(item.goldenUuid) }} >
                                        <img src={datasetSegImgAPI(item.segmentUuid)} onError={replaceImage} style={{ maxHeight: 44, maxWidth: 44 }} />
                                    </div>
                                </div>
                                <div className='my-tbody-td' style={{ width: tableColumnWidth[4] }} onMouseEnter={() => { setHoverItem1(item.segmentUuid); setHoverItem2(item.goldenUuid) }} >
                                    <div className='my-thumb-image-container'>
                                        <img src={datasetSegImgAPI(item.goldenUuid)} onError={replaceImage} style={{ maxHeight: 44, maxWidth: 44 }} />
                                    </div>
                                </div>
                                ...
                                <div className='my-tbody-td' style={{ width: tableColumnWidth[8] }} >
                                    <ToggleButton id={'toggle_' + item.segmentUuid} status={(item.label === "PASS") ? "run" : "stop"}
                                        onChange={() => handleLabelToggle(item.segmentUuid)}
                                        top={-6} left={0}
                                    ></ToggleButton>
                                </div>
...
export default ResultTableListAspect;
