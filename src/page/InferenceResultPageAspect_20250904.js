...
import { filter, toArray, findIndex, isEqual, map, cloneDeep, sortBy, orderBy, take, slice, uniqBy, set } from 'lodash-es';
...
const InferenceResultPageAspect = (props) => {
    ...
    const jsonDataGroupby = (dataArray, columnName) => {
        // return dataArray.reduce((acc, obj) => {
        //     const key = obj[columnName];
        //     if (!acc[key]) {
        //         acc[key] = [];
        //     }
        //     acc[key].push(obj);
        ...
        const groupBy = prop => dataArray => {
            return dataArray.reduce((dict, item) => {
        ...
        const result = Object.entries(groupBy(columnName)(dataArray))
    ...
    const fetchCSV = async (myTaoModelId) => {
        ...
        console.log('fetch csv', myTaoModelId);
        ...
        try {
            ...
            console.log('CSV Result:', resultArr);


            let resultArrSort = orderBy(resultArr, ['image_uuid', 'segment_index'], ['asc', 'asc']);
            //resultArrSort.forEach((_, i, a) => a[i].sortIndex = i);

            console.log('CSV Result Sort:', resultArrSort);
    ...
    const fetchCSV_xx = async (myTaoModelId) => {

        try {

            // `${localhost}${port}${nginx_proxy}/tao/rest/${trainv1}/modelWorkspace/inference/result?tao_model_uuid=${myTaoModelId}`;
            ...
            dataArr.map((item, index) => {
                ...
                if (index > 0) {
                    ...
                    const compInfo = lineArr[0].split('/');

                    //const compName = lineArr[0].substring(0, lineArr[0].indexOf('/'));
                    const compName = compInfo[0];
                    const lightSource = compInfo[3];

                    //log(`compName=${compName}`);
                    const segmentIndex = compInfo[4];
                    ...
                    const exportUuid = uuidList[0];
                    const goldenUuid = uuidList[1];
                    const segmentUuid = uuidList[2];


                    const dataUuidArr = uuidList[2].split('_');

                    console.log('----------------------dataUuidArr');
                    console.log(dataUuidArr);

                    //const datasetUuid = uuidList[0];

                    console.log('===========aaa==========')
                    log(`exportUuid=${exportUuid}`);
                    log(`goldenUuid=${goldenUuid}`);
                    log(`segmentUuid=${segmentUuid}`);
                    console.log('==========bbb===========')
                    ...
                    myData.segmentIndex = segmentIndex;
                    myData.exportUuid = exportUuid; //`${exportUuid}_${goldenUuid}_${segmentUuid}`;
                    myData.goldenUuid = goldenUuid;
                    myData.segmentUuid = segmentUuid;
            ...
            let resultArrSort = orderBy(resultArr, ['compName', 'lightSource', 'imageUuid', 'segmentUuid'], ['asc', 'asc', 'asc', 'asc']);
            resultArrSort.forEach((_, i, a) => a[i].sortIndex = i);
            ...
            const jsonDataGrouped = jsonDataGroupby(resultArrSort, 'imageUuid');
            console.log('jsonDataGrouped ==================>');
            console.log(jsonDataGrouped);
                        ...
                        <div className='container-fluid'>
                            ...
                            <div className='row'>
                                <div className='col-12 p-0 my-dialog-content mt-3'>
                                    {
                                        (resultView === 'card') ?
                                            <ResultCardListAspect data={slice(filterList, (currentPage - 1) * pageNum, ((currentPage - 1) * pageNum) + pageNum)} onChange={handleLabelToggle} resultList={resultList} />
                                            :
                                            <ResultTableListAspect data={slice(filterList, (currentPage - 1) * pageNum, ((currentPage - 1) * pageNum) + pageNum)} onChange={handleLabelToggle} />
                                    }
