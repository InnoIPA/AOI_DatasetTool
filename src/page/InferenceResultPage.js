...
import { postTrainPassAPI, postTrainNgAPI } from '../APIPath';
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (passArr.length > 0) {
                ...
                const resPass = await fetch(postTrainPassAPI, {
                    method: 'PUT',
                    headers: {
                        'Content-Type': 'application/json',
                    },
            ...
            if (ngArr.length > 0) {
                ...
                const resNg = await fetch(postTrainNgAPI, {
                    method: 'PUT',
                    headers: {
                        'Content-Type': 'application/json',
                    },
        ...
        try {
            ...
            dataArr.map((item, index) => {
                ...
                if (index > 0) {
                    ...
                    log(`score=${score}`);
                    const threshold = lineArr[5];
                    log(`threshold=${threshold}`);
                    const infer_label = lineArr[6];
                    log(`infer_label=${infer_label}`);
                    ...
                    myData.score = score;
                    myData.threshold = threshold;
                    myData.infer_label = infer_label;
                        ...
                        <div className='container-fluid'>
                            <div className='row'>
                                <div className='col-12 p-0 my-dialog-title d-flex flex-row justify-content-between'>
                                    <ProjectNamePanel projectName={utilityRef.current?.getCurrentTaoModelName()} aspect={false} />
