...
export const theme = createTheme({
    palette: {
        primary: {
            main: window.getComputedStyle(document.documentElement).getPropertyValue('--myRedColor').trim(),
        },
        secondary: {
            main: '#888',
        },
...
export const theme2 = createTheme({
    palette: {
        primary: {
            main: window.getComputedStyle(document.documentElement).getPropertyValue('--myBlueColor').trim(),
        },
        secondary: {
            main: '#888',
        },
...
const ProjectPage = (props: ProjectPageProps) => {
    ...
    const handleClickProject_xx = (project: ProjectDataType, runningCopyToLocal: boolean, runningConvert: boolean) => {
    ...
    const handleClickProject = (e: MouseEvent<HTMLButtonElement>, project: ProjectDataType) => {

        console.log('current project')
        console.log(project)


        const runningCopyToLocal = (project.project_status.copy_to_local.status === 'running');
        const runningConvert = (project.project_status.generate_zip?.status === 'running');

        setCurrentProject(project);
        if (runningCopyToLocal) return setPageKey('LoadingCopyToLocalPage');
        if (runningConvert) return setPageKey('LoadingPanelDatasetZipPage');
        if (project.export_uuid) {
            if (project.project_high_aspect) {
                return setPageKey('SetAttributePageAspect');
            } else {
                return setPageKey('SetAttributePage');
            }
        }
            ...
            <div className="container">
                <div className="title-container first-title-container">
                    ...
                    <div className='d-flex flex-row gap-2 align-items-center' style={{ height: 40 }}>
                        <CustomButton name="add-project-aspect" text="Add Finger Project" width={210} height={42} onClick={handleClickAddAspectBtn} />
                        <CustomButton name="add-project" text="Add General Project" width={210} height={42} onClick={handleClickAddBtn} />
                    </div>
                </div>
                ...
                <div className="project-wrapper">
                    {projectData.length > 0 &&
                        projectData.map((project,myIndex) => (

                            <ProjectCard
                                project={project}
                                key={`project_${myIndex}`}
